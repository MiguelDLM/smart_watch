        ...
        if (list != null) {
            try {
                if (list.size() != 0) {
                    ...
                    if (!(iTanxTableScreenExpressAd == null || iTanxTableScreenExpressAd.getBidInfo() == null || iTanxTableScreenExpressAd.getBidInfo().getTemplateConf() == null)) {
                        if (!TextUtils.isEmpty(iTanxTableScreenExpressAd.getBidInfo().getTemplateConf().getPidStyleId())) {
                            return;
                        }
                    }
                ...
                if (onAdLoadListener != null) {
                    onAdLoadListener.onError(new TanxError("缓存try - catch异常: " + LogUtils.getStackTraceMessage(e)));
                    return;
                }
                return;
            }
