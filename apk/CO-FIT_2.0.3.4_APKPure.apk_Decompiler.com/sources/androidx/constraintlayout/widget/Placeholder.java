...
public class Placeholder extends View {
    ...
    public void onDraw(@NonNull Canvas canvas) {
        if (isInEditMode()) {
            ...
            canvas.drawText(LocationInfo.NA, ((((float) width) / 2.0f) - (((float) rect.width()) / 2.0f)) - ((float) rect.left), ((((float) height) / 2.0f) + (((float) rect.height()) / 2.0f)) - ((float) rect.bottom), paint);
        }
    }
