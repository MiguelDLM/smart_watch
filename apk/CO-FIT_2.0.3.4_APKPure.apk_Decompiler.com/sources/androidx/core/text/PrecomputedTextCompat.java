...
public class PrecomputedTextCompat implements Spannable {
    ...
    public PrecomputedText getPrecomputedText() {
        if (xxIXOIIO.oIX0oI(this.mText)) {
            return xoIox.oIX0oI(this.mText);
        }
        return null;
    }
    ...
    public static final class Params {
        ...
        public Params(@NonNull TextPaint textPaint, @NonNull TextDirectionHeuristic textDirectionHeuristic, int i, int i2) {
            if (Build.VERSION.SDK_INT >= 29) {
                this.mWrapped = oO.oIX0oI(textPaint).setBreakStrategy(i).setHyphenationFrequency(i2).setTextDirection(textDirectionHeuristic).build();
            } else {
                this.mWrapped = null;
            }
            this.mPaint = textPaint;
