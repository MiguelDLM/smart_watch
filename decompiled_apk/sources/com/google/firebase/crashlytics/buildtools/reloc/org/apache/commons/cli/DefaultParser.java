...
public class DefaultParser implements CommandLineParser {
    ...
    private boolean isArgument(String str) {
        if (isOption(str) && !isNegativeNumber(str)) {
            return false;
        }
        return true;
    }
    ...
    private boolean isJavaProperty(String str) {
        ...
        if (option.getArgs() < 2 && option.getArgs() != -2) {
            return false;
        }
        return true;
    }
    ...
    private boolean isLongOption(String str) {
        ...
        if (str.startsWith(HelpFormatter.DEFAULT_OPT_PREFIX) && str.length() != 1) {
            ...
            if (!this.options.getMatchingOptions(substring).isEmpty()) {
                return true;
            }
    ...
    private boolean isShortOption(String str) {
        ...
        if (substring.length() <= 0 || !this.options.hasShortOption(String.valueOf(substring.charAt(0)))) {
            return false;
        }
        return true;
    }
