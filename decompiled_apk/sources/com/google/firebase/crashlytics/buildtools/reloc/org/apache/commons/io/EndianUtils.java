...
public class EndianUtils {
    ...
    public static long readSwappedLong(byte[] bArr, int i) {
        return (readSwappedInteger(bArr, i + 4) << 32) + (readSwappedInteger(bArr, i) & BleWatchFaceId.WATCHFACE_ID_INVALID);
    }
    ...
    public static long readSwappedUnsignedInteger(byte[] bArr, int i) {
        return ((bArr[i + 3] & 255) << 24) + (((bArr[i] & 255) + ((bArr[i + 1] & 255) << 8) + ((bArr[i + 2] & 255) << 16)) & BleWatchFaceId.WATCHFACE_ID_INVALID);
    }
    ...
    public static long readSwappedUnsignedInteger(InputStream inputStream) throws IOException {
        return ((read(inputStream) & 255) << 24) + (((read(inputStream) & 255) + ((read(inputStream) & 255) << 8) + ((read(inputStream) & 255) << 16)) & BleWatchFaceId.WATCHFACE_ID_INVALID);
    }
