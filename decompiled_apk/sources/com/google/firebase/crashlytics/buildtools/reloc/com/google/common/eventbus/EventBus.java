...
public class EventBus {
    ...
    public static final class LoggingHandler implements SubscriberExceptionHandler {
        ...
        private static Logger logger(SubscriberExceptionContext subscriberExceptionContext) {
            return Logger.getLogger(EventBus.class.getName() + FileUtils.FILE_EXTENSION_SEPARATOR + subscriberExceptionContext.getEventBus().identifier());
        }
        ...
        private static String message(SubscriberExceptionContext subscriberExceptionContext) {
            ...
            return "Exception thrown by subscriber method " + subscriberMethod.getName() + HexStringBuilder.COMMENT_BEGIN_CHAR + subscriberMethod.getParameterTypes()[0].getName() + HexStringBuilder.COMMENT_END_CHAR + " on subscriber " + subscriberExceptionContext.getSubscriber() + " when dispatching event: " + subscriberExceptionContext.getEvent();
        }
