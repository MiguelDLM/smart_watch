...
public class C40Encoder implements Encoder {
    ...
    private static String encodeToCodewords(CharSequence charSequence, int i) {
        int charAt = (charSequence.charAt(i) * 1600) + (charSequence.charAt(i + 1) * HexStringBuilder.COMMENT_BEGIN_CHAR) + charSequence.charAt(i + 2) + 1;
    ...
    public int encodeChar(char c, StringBuilder sb) {
        ...
        if (c >= '!' && c <= '/') {
            sb.append((char) 1);
            sb.append((char) (c - oOoXoXO.f2346I0Io));
            return 2;
        }
        if (c >= ':' && c <= '@') {
            sb.append((char) 1);
