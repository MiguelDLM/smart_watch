...
public class HctSolver {
    ...
    public static double[][] bisectToSegment(double d, double d2) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            if (nthVertex[0] >= XIXIX.OOXIXo.f3760XO) {
    ...
    public static int findResultByJ(double d, double d2, double d3) {
        ...
        while (i < 5) {
            ...
            if (d2 != XIXIX.OOXIXo.f3760XO && sqrt != XIXIX.OOXIXo.f3760XO) {
                d4 = d2 / Math.sqrt(d6);
            } else {
                d4 = 0.0d;
            }
            ...
            double aw = (viewingConditions.getAw() * Math.pow(d6, (d5 / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double d9 = (((0.305d + aw) * 23.0d) * pow2) / (((23.0d * d8) + ((pow2 * 11.0d) * cos2)) + ((pow2 * 108.0d) * sin));
            double d10 = d9 * cos2;
            double d11 = d9 * sin;
            double d12 = aw * 460.0d;
            double[] matrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation((((451.0d * d10) + d12) + (288.0d * d11)) / 1403.0d), inverseChromaticAdaptation(((d12 - (891.0d * d10)) - (261.0d * d11)) / 1403.0d), inverseChromaticAdaptation(((d12 - (d10 * 220.0d)) - (d11 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            double d13 = matrixMultiply[0];
            if (d13 < XIXIX.OOXIXo.f3760XO) {
                break;
            }
            double d14 = matrixMultiply[1];
            if (d14 < XIXIX.OOXIXo.f3760XO) {
                break;
            }
            double d15 = matrixMultiply[2];
            if (d15 < XIXIX.OOXIXo.f3760XO) {
                break;
            }
            double[] dArr = Y_FROM_LINRGB;
            double d16 = (dArr[0] * d13) + (dArr[1] * d14) + (dArr[2] * d15);
            if (d16 <= XIXIX.OOXIXo.f3760XO) {
                return 0;
            }
            if (i != 4) {
                double d17 = d16 - d3;
    ...
    public static double inverseChromaticAdaptation(double d) {
        ...
        return MathUtils.signum(d) * Math.pow(Math.max(XIXIX.OOXIXo.f3760XO, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d);
    }

    public static boolean isBounded(double d) {
        return XIXIX.OOXIXo.f3760XO <= d && d <= 100.0d;
    }
