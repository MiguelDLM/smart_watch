...
public abstract class tanxc_if<Req extends RequestBaseBean, Rep extends BaseBean> {
    ...
    public void request(final Req req, final NetWorkCallBack<Rep> netWorkCallBack) {
        com.alimm.tanx.core.common.tanxc_do.tanxc_do(new Runnable() { // from class: com.alimm.tanx.core.request.tanxc_if.1
                ...
                try {
                    if (tanxc_if.this.checkInitData(req, netWorkCallBack)) {
                        ...
                        if (buildRequestBean == null) {
                            ...
                            if (tanxc_if.this.checkDeviceId(req)) {
                                if (tanxc_if.this.checkRequestBefore(req, netWorkCallBack)) {
                            ...
                            netWorkCallBack3.error(utErrorCode2.getIntCode(), "", utErrorCode2.getMsg());
                        }
                    }
                } catch (Exception e) {
    ...
    public String uuid() {
        return UUID.randomUUID().toString().replace(HelpFormatter.DEFAULT_OPT_PREFIX, "") + HelpFormatter.DEFAULT_OPT_PREFIX + (System.currentTimeMillis() / 1000);
    }
}
