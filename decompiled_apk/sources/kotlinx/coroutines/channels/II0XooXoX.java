package kotlinx.coroutines.channels;

import java.util.concurrent.CancellationException;
import kotlin.DeprecationLevel;
import kotlin.coroutines.CoroutineContext;
import kotlin.jvm.internal.XX;
import kotlin.oXIO0o0XI;
import kotlinx.coroutines.JobCancellationException;
import kotlinx.coroutines.JobSupport;

@XX({"SMAP\nChannelCoroutine.kt\nKotlin\n*S Kotlin\n*F\n+ 1 ChannelCoroutine.kt\nkotlinx/coroutines/channels/ChannelCoroutine\n+ 2 JobSupport.kt\nkotlinx/coroutines/JobSupport\n*L\n1#1,41:1\n706#2,2:42\n706#2,2:44\n706#2,2:46\n*S KotlinDebug\n*F\n+ 1 ChannelCoroutine.kt\nkotlinx/coroutines/channels/ChannelCoroutine\n*L\n21#1:42,2\n26#1:44,2\n32#1:46,2\n*E\n"})
/* loaded from: classes6.dex */
public class II0XooXoX<E> extends kotlinx.coroutines.oIX0oI<oXIO0o0XI> implements Oxx0IOOO<E> {

    /* renamed from: Oxx0xo, reason: collision with root package name */
    @OXOo.OOXIXo
    public final Oxx0IOOO<E> f29861Oxx0xo;

    public II0XooXoX(@OXOo.OOXIXo CoroutineContext coroutineContext, @OXOo.OOXIXo Oxx0IOOO<E> oxx0IOOO, boolean z, boolean z2) {
        super(coroutineContext, z, z2);
        this.f29861Oxx0xo = oxx0IOOO;
    }

    @Override // kotlinx.coroutines.JobSupport
    public void I0oOIX(@OXOo.OOXIXo Throwable th) {
        CancellationException XI2 = JobSupport.XI(this, th, null, 1, null);
        this.f29861Oxx0xo.II0xO0(XI2);
        XoI0Ixx0(XI2);
    }

    @Override // kotlinx.coroutines.JobSupport, kotlinx.coroutines.oXIO0o0XI
    public final void II0xO0(@OXOo.oOoXoXO CancellationException cancellationException) {
        if (isCancelled()) {
            return;
        }
        if (cancellationException == null) {
            cancellationException = new JobCancellationException(IoOoo(), null, this);
        }
        I0oOIX(cancellationException);
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    @OXOo.OOXIXo
    public Object IIXOooo() {
        return this.f29861Oxx0xo.IIXOooo();
    }

    @Override // kotlinx.coroutines.channels.oI0IIXIo
    public void OOXIXo(@OXOo.OOXIXo Oox.oOoXoXO<? super Throwable, oXIO0o0XI> oooxoxo) {
        this.f29861Oxx0xo.OOXIXo(oooxoxo);
    }

    @OXOo.OOXIXo
    public final Oxx0IOOO<E> OooI() {
        return this.f29861Oxx0xo;
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    @OXOo.OOXIXo
    public kotlinx.coroutines.selects.X0o0xo<xoIox<E>> Oxx0xo() {
        return this.f29861Oxx0xo.Oxx0xo();
    }

    @Override // kotlinx.coroutines.channels.oI0IIXIo
    public boolean X0IIOO() {
        return this.f29861Oxx0xo.X0IIOO();
    }

    @Override // kotlinx.coroutines.channels.oI0IIXIo
    @OXOo.oOoXoXO
    public Object XOxIOxOx(E e, @OXOo.OOXIXo kotlin.coroutines.I0Io<? super oXIO0o0XI> i0Io) {
        return this.f29861Oxx0xo.XOxIOxOx(e, i0Io);
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    @OXOo.OOXIXo
    public kotlinx.coroutines.selects.X0o0xo<E> Xx000oIo() {
        return this.f29861Oxx0xo.Xx000oIo();
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    public boolean XxX0x0xxx() {
        return this.f29861Oxx0xo.XxX0x0xxx();
    }

    @Override // kotlinx.coroutines.JobSupport, kotlinx.coroutines.oXIO0o0XI
    @kotlin.OOXIXo(level = DeprecationLevel.HIDDEN, message = "Since 1.2.0, binary compatibility with versions <= 1.1.x")
    public /* synthetic */ void cancel() {
        I0oOIX(new JobCancellationException(IoOoo(), null, this));
    }

    @OXOo.OOXIXo
    public final Oxx0IOOO<E> getChannel() {
        return this;
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    public boolean isEmpty() {
        return this.f29861Oxx0xo.isEmpty();
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    @OXOo.OOXIXo
    public ChannelIterator<E> iterator() {
        return this.f29861Oxx0xo.iterator();
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    @OXOo.oOoXoXO
    public Object o00(@OXOo.OOXIXo kotlin.coroutines.I0Io<? super xoIox<? extends E>> i0Io) {
        Object o002 = this.f29861Oxx0xo.o00(i0Io);
        kotlin.coroutines.intrinsics.II0xO0.oOoXoXO();
        return o002;
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    @OXOo.OOXIXo
    public kotlinx.coroutines.selects.X0o0xo<E> oI0IIXIo() {
        return this.f29861Oxx0xo.oI0IIXIo();
    }

    @Override // kotlinx.coroutines.JobSupport, kotlinx.coroutines.oXIO0o0XI
    @kotlin.OOXIXo(level = DeprecationLevel.HIDDEN, message = "Since 1.2.0, binary compatibility with versions <= 1.1.x")
    public final /* synthetic */ boolean oIX0oI(Throwable th) {
        I0oOIX(new JobCancellationException(IoOoo(), null, this));
        return true;
    }

    @Override // kotlinx.coroutines.channels.oI0IIXIo
    @kotlin.OOXIXo(level = DeprecationLevel.ERROR, message = "Deprecated in the favour of 'trySend' method", replaceWith = @kotlin.XX(expression = "trySend(element).isSuccess", imports = {}))
    public boolean offer(E e) {
        return this.f29861Oxx0xo.offer(e);
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    @kotlin.OOXIXo(level = DeprecationLevel.ERROR, message = "Deprecated in the favour of 'tryReceive'. Please note that the provided replacement does not rethrow channel's close cause as 'poll' did, for the precise replacement please refer to the 'poll' documentation", replaceWith = @kotlin.XX(expression = "tryReceive().getOrNull()", imports = {}))
    @OXOo.oOoXoXO
    public E poll() {
        return this.f29861Oxx0xo.poll();
    }

    @Override // kotlinx.coroutines.channels.oI0IIXIo
    public boolean x0o(@OXOo.oOoXoXO Throwable th) {
        return this.f29861Oxx0xo.x0o(th);
    }

    @Override // kotlinx.coroutines.channels.oI0IIXIo
    @OXOo.OOXIXo
    public Object x0xO0oo(E e) {
        return this.f29861Oxx0xo.x0xO0oo(e);
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    @OXOo.oOoXoXO
    public Object xII(@OXOo.OOXIXo kotlin.coroutines.I0Io<? super E> i0Io) {
        return this.f29861Oxx0xo.xII(i0Io);
    }

    @Override // kotlinx.coroutines.channels.ReceiveChannel
    @kotlin.OOXIXo(level = DeprecationLevel.ERROR, message = "Deprecated in favor of 'receiveCatching'. Please note that the provided replacement does not rethrow channel's close cause as 'receiveOrNull' did, for the detailed replacement please refer to the 'receiveOrNull' documentation", replaceWith = @kotlin.XX(expression = "receiveCatching().getOrNull()", imports = {}))
    @xx0o0O.II0XooXoX
    @OXOo.oOoXoXO
    public Object xoXoI(@OXOo.OOXIXo kotlin.coroutines.I0Io<? super E> i0Io) {
        return this.f29861Oxx0xo.xoXoI(i0Io);
    }

    @Override // kotlinx.coroutines.channels.oI0IIXIo
    @OXOo.OOXIXo
    public kotlinx.coroutines.selects.Oxx0IOOO<E, oI0IIXIo<E>> xxIXOIIO() {
        return this.f29861Oxx0xo.xxIXOIIO();
    }
}
