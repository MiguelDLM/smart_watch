...
// Decompress image data using RLE-like decompression
unsigned char* decompress_image_data(unsigned char* compressed_data, int compressed_size, int width, int height, int* output_size) {
    // Calculate expected output size (4 bytes per pixel for RGBA)
    ...
    while (input_pos < compressed_size && output_pos < expected_size) {
        if (input_pos + 1 >= compressed_size) break;
        
        unsigned char count = compressed_data[input_pos++];
        unsigned char value = compressed_data[input_pos++];
        
        // Handle run-length encoding
        for (int i = 0; i < count && output_pos < expected_size; i++) {
...
void extract_image_data(int block_index, dial_block_t *block, const char *output_dir) {
        ...
        case 0x98: // Battery strip
            {
                ...
                for (int i = 0; i < block->parts; i++) {
                    ...
                    int offset = i * digit_size;
                    
                    // Extract compressed data and try to decompress
                    ...
                    unsigned char *decompressed_data = decompress_image_data(compressed_data, digit_size, 
                                                                           digit_width, digit_height, &decompressed_size);
    ...
    if (g_picture_sizes[block->picidx] > 0) {
        ...
        unsigned char *decompressed_data = decompress_image_data(compressed_data, g_picture_sizes[block->picidx], 
                                                               block->sx, block->sy, &decompressed_size);
